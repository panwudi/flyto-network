#!/usr/bin/env bash
# ============================================================
# modules/hk-setup.sh — 香港中转节点部署模块 v2
#
# 重构改动：
#   - 顶部加 set -euo pipefail（原缺失）
#   - PANEL_IP 为空时不再生成空路由规则（原 bug）
#   - HK_PUB_IP 探测失败时强制要求手动输入（原 bug）
#   - wg0.conf DNS 行逻辑修正（原逻辑矛盾）
#   - 所有输入接入 validate.sh 进行格式校验
#   - 进度追踪接入 progress.sh
#   - 错误处理接入 error.sh（trap ERR）
#   - update-panel-route.sh 中 GW 变量兜底修复
#   - 备份中 wg-was-up 恢复逻辑修复
# ============================================================
set -euo pipefail

# ── 加载 lib（支持从 flyto.sh source 或独立运行两种方式）
_HK_MODULE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_HK_LIB_DIR="${_HK_MODULE_DIR}/../lib"

for _lib in ui.sh validate.sh progress.sh error.sh; do
  # shellcheck disable=SC1090
  [[ -f "${_HK_LIB_DIR}/${_lib}" ]] && source "${_HK_LIB_DIR}/${_lib}"
done
...
if ! command -v ui_info >/dev/null 2>&1; then
  ...
  ui_confirm() { local a; read -r -p "  $1 [y/N]: " a </dev/tty; [[ "${a}" =~ ^[Yy] ]]; }
fi

if ! command -v progress_step >/dev/null 2>&1; then
  progress_step() { ui_step "步骤 $1/$_STEP_TOTAL：$2"; }
...
if ! command -v validate_ipv4_cidr >/dev/null 2>&1; then
  ...
  validate_input_loop_strict() {
    local __var="$1" label="$2" default="$3" validator="$4"
    local val=""
    while true; do
      printf "  %s [%s]: " "${label}" "${default}" >/dev/tty
      IFS= read -r val </dev/tty || return 1
      [[ -z "${val}" ]] && val="${default}"
      "${validator}" "${val}" 2>/dev/null && break
      ui_warn "输入无效，请重新输入"
    done
...
if ! command -v error_trap_install >/dev/null 2>&1; then
  error_trap_install() {
    trap 'echo "[ERR] 命令失败: ${BASH_COMMAND} (exit $?) @ ${BASH_SOURCE[0]}:${LINENO}" >&2' ERR
  }
...
_step_base_system() {
  ...
  # 软件源更新（失败不中止，warn 即可）
  error_trap_remove
  ui_spin "更新软件源索引" apt-get update -y || ui_warn "软件源更新失败，继续使用现有索引"
  error_trap_install
...
_step_collect_network() {
  _STEP_NAME="采集本机网络信息"
  progress_step 2 "${_STEP_NAME}"

  # 若 wg0 运行中，暂停以获取本机真实网络信息
  if ip link show wg0 >/dev/null 2>&1 && ip link show wg0 | grep -q 'UP'; then
    ui_warn "检测到 wg0 运行中，暂停以探测本机真实 IP..."
  ...
  # 多源探测公网 IP，任一成功即可
  HK_PUB_IP=""
  for probe_url in https://ifconfig.io https://ip.sb https://api4.my-ip.io/ip; do
    HK_PUB_IP="$(curl -4 -s --max-time 8 "${probe_url}" 2>/dev/null | tr -d '[:space:]' || true)"
    validate_ipv4 "${HK_PUB_IP}" 2>/dev/null && break || HK_PUB_IP=""
  done

  ui_info "自动探测结果："
  echo "    WAN 接口 : ${HK_WAN_IF:-<未检测到，需手动输入>}"
  echo "    默认网关 : ${HK_GW:-<未检测到，需手动输入>}"
  echo "    公网 IP  : ${HK_PUB_IP:-<未检测到，需手动输入>}"
  echo
  ...
  validate_input_loop_strict HK_WAN_IF \
    ...
    "${HK_WAN_IF}" \
    validate_iface \
    "提示：运行 ip -o -4 route show to default 可查看"
  ...
  validate_input_loop_strict HK_GW \
    ...
    "${HK_GW}" \
    validate_ipv4 \
    "提示：运行 ip route show default 可查看"

  # 公网 IP：带 IPv4 格式校验，探测失败时强制输入（不允许跳过）
  if [[ -z "${HK_PUB_IP}" ]]; then
    ui_warn "公网 IP 自动探测失败，必须手动输入"
  fi
  ...
  echo
  ui_ok "网络信息确认：WAN=${HK_WAN_IF}  GW=${HK_GW}  PubIP=${HK_PUB_IP}"
}

# ============================================================
# 步骤 3a：WireGuard 参数输入（全新安装）
# ============================================================
HK_PRIV_KEY="" HK_PUB_KEY="" HK_WG_ADDR=""
US_PUB_KEY="" US_WG_ENDPOINT="" US_WG_TUN_IP=""
HK_WG_KEEPALIVE=25 V2BX_NODE_ID=""

_input_wg_fresh() {
  _STEP_NAME="输入 WireGuard 配置"
  progress_step 3 "${_STEP_NAME}"

  while true; do
    echo
    ui_info "请在美国节点执行 wg show 获取公钥与 Endpoint"
    echo
    ...
    validate_input_loop HK_WG_KEEPALIVE \
      "WG PersistentKeepalive（秒，建议 25）" \
      "${HK_WG_KEEPALIVE:-25}" \
      validate_keepalive 2>/dev/null || HK_WG_KEEPALIVE=25

    # 派生公钥
    HK_PUB_KEY="$(printf '%s' "${HK_PRIV_KEY}" | wg pubkey 2>/dev/null || true)"
    if [[ -z "${HK_PUB_KEY}" ]]; then
      ui_warn "无法从私钥派生公钥，请确认私钥格式正确"
    fi

    # 确认摘要
    echo
    echo -e "  ┌─────────────────────────────────────────────────┐"
    echo -e "  │  ${W}WireGuard 参数确认${N}"
    echo -e "  ├─────────────────────────────────────────────────┤"
    printf  "  │  %-20s : %s\n" "HK 私钥" "$(_ui_mask_secret "${HK_PRIV_KEY}" 2>/dev/null || echo "${HK_PRIV_KEY:0:4}...")"
    printf  "  │  %-20s : %s\n" "HK WG 地址" "${HK_WG_ADDR}"
    printf  "  │  %-20s : %s\n" "US 公钥" "${US_PUB_KEY:0:12}..."
    printf  "  │  %-20s : %s\n" "US Endpoint" "${US_WG_ENDPOINT}"
    printf  "  │  %-20s : %s\n" "US 隧道 IP" "${US_WG_TUN_IP}"
    printf  "  │  %-20s : %s\n" "Keepalive" "${HK_WG_KEEPALIVE}s"
    printf  "  │  %-20s : %s\n" "V2bX 节点 ID" "${V2BX_NODE_ID}"
    echo -e "  └─────────────────────────────────────────────────┘"
    echo

    local gate_rc=0
    progress_gate "确认无误，继续部署？" || gate_rc=$?
    case "${gate_rc}" in
      0) return 0 ;;
...
_input_wg_restore() {
  ...
  while true; do
    echo
    ui_info "请粘贴备份块（以 END 行、连续两次空行或 Ctrl+D 结束）"
    echo -e "  示例字段：HK_PRIV_KEY=xxx  HK_WG_ADDR=10.0.0.3/32 ..."
    echo -e "  ${D}─── 开始粘贴 ──────────────────────────────────────${N}"
    ...
    US_PUB_KEY="" US_WG_ENDPOINT="" US_WG_TUN_IP=""
    HK_WAN_IF="" HK_GW="" HK_PUB_IP=""
    V2BX_NODE_ID="" HK_WG_KEEPALIVE="25"
    ...
    while IFS= read -r line; do
      ...
      case "${k}" in
        ...
        HK_WG_KEEPALIVE)               HK_WG_KEEPALIVE="${v}"; parsed_count=$((parsed_count+1)) ;;
        US_WG_TUN_IP|US_WG_ADDR)       US_WG_TUN_IP="${v}";    parsed_count=$((parsed_count+1)) ;;
        HK_WAN_IF)                     HK_WAN_IF="${v}";        parsed_count=$((parsed_count+1)) ;;
        HK_GW)                         HK_GW="${v}";            parsed_count=$((parsed_count+1)) ;;
        HK_PUB_IP)                     HK_PUB_IP="${v}";        parsed_count=$((parsed_count+1)) ;;
        V2BX_NODE_ID|NODE_ID)          V2BX_NODE_ID="${v}";    parsed_count=$((parsed_count+1)) ;;
      esac
    done <<< "${lines}"

    ui_info "识别到 ${parsed_count} 个字段"
    ...
    if [[ -z "${HK_PRIV_KEY}" ]] || ! validate_wg_key "${HK_PRIV_KEY}" 2>/dev/null; then
      ...
      validate_input_loop_strict HK_PRIV_KEY "香港节点 WG 私钥" "${HK_PRIV_KEY:-}" validate_wg_key || return 1
    fi
    if [[ -z "${HK_WG_ADDR}" ]] || ! validate_ipv4_cidr "${HK_WG_ADDR}" 2>/dev/null; then
      ui_warn "HK_WG_ADDR 缺失或格式错误，请手动输入"
      validate_input_loop_strict HK_WG_ADDR "香港节点 WG 隧道地址" "${HK_WG_ADDR:-}" validate_ipv4_cidr || return 1
    fi
    if [[ -z "${US_PUB_KEY}" ]] || ! validate_wg_key "${US_PUB_KEY}" 2>/dev/null; then
      ui_warn "US_PUB_KEY 缺失或格式错误，请手动输入"
      validate_input_loop_strict US_PUB_KEY "美国节点 WG 公钥" "${US_PUB_KEY:-}" validate_wg_key || return 1
    fi
    if [[ -z "${US_WG_ENDPOINT}" ]] || ! validate_wg_endpoint "${US_WG_ENDPOINT}" 2>/dev/null; then
      ui_warn "US_WG_ENDPOINT 缺失或格式错误，请手动输入"
      validate_input_loop_strict US_WG_ENDPOINT "美国节点 WG Endpoint" "${US_WG_ENDPOINT:-}" validate_wg_endpoint || return 1
    fi
    if [[ -z "${US_WG_TUN_IP}" ]] || ! validate_ipv4_cidr "${US_WG_TUN_IP}" 2>/dev/null; then
      # 尝试从本地地址推断
      if [[ -n "${HK_WG_ADDR}" ]]; then
        local guessed
        guessed="$(_hk_guess_peer_tun_ip "${HK_WG_ADDR}" 2>/dev/null || true)"
        [[ -n "${guessed}" ]] && ui_warn "US_WG_TUN_IP 未提供，已按 ${HK_WG_ADDR} 推测为 ${guessed}"
        US_WG_TUN_IP="${guessed:-}"
      fi
      validate_input_loop_strict US_WG_TUN_IP "美国节点 WG 隧道 IP" "${US_WG_TUN_IP:-10.0.0.1/32}" validate_ipv4_cidr || return 1
    fi
    if [[ -z "${V2BX_NODE_ID}" ]] || ! validate_positive_integer "${V2BX_NODE_ID}" 2>/dev/null; then
      ui_warn "V2BX_NODE_ID 缺失或非数字，请手动输入"
      validate_input_loop V2BX_NODE_ID "V2bX 节点 ID（纯数字）" "${V2BX_NODE_ID:-}" validate_positive_integer || return 1
    fi
    ...
    printf  "  │  %-20s : %s\n" "Keepalive" "${HK_WG_KEEPALIVE}s"
    printf  "  │  %-20s : %s\n" "V2bX 节点 ID" "${V2BX_NODE_ID}"
    [[ -n "${HK_WAN_IF}" ]] && printf "  │  %-20s : %s\n" "WAN 接口（备份）" "${HK_WAN_IF}"
    [[ -n "${HK_GW}" ]]     && printf "  │  %-20s : %s\n" "网关（备份）" "${HK_GW}"
    [[ -n "${HK_PUB_IP}" ]] && printf "  │  %-20s : %s\n" "公网 IP（备份）" "${HK_PUB_IP}"
    echo -e "  └─────────────────────────────────────────────────┘"
    echo

    local gate_rc=0
    progress_gate "确认无误，继续恢复部署？" || gate_rc=$?
    case "${gate_rc}" in
      0) return 0 ;;
      2) ui_info "重新粘贴备份块" ;;
      3) exit 0 ;;
...
# ============================================================
# 步骤 4：生成 wg0.conf + 启动 + 三项验证
# ============================================================
_step_setup_wireguard() {
  _STEP_NAME="配置 WireGuard"
  progress_step 4 "${_STEP_NAME}"
  error_trap_install

  local US_PUB_IP="${US_WG_ENDPOINT%%:*}"

  # ── 解析面板 IP（修复：为空时生成空路由的 bug）──────
  local PANEL_IP=""
  ...
  if [[ -z "${PANEL_IP}" ]]; then
    ui_warn "无法解析面板域名 ${panel_host}，将跳过面板路由（部署后可手动执行 update-panel-route.sh）"
  else
    ui_info "面板 IP：${PANEL_IP}"
    validate_ipv4 "${PANEL_IP}" 2>/dev/null || {
      ui_warn "面板 IP 格式异常（${PANEL_IP}），已跳过面板路由"
      PANEL_IP=""
    }
  fi

  # ── DNS 行：禁用 systemd-resolved 后直接不写 DNS──────
  # （wg-quick 的 DNS 钩子依赖 resolvconf 或 systemd-resolved，
  #  两者均已被本脚本禁用，写 DNS 行反而会导致 PostUp 报错）
  local WG_DNS_LINE=""
  # 仅当 resolvconf 存在且 systemd-resolved 未被 mask 时才写
  if command -v resolvconf >/dev/null 2>&1 \
  && ! systemctl is-enabled systemd-resolved >/dev/null 2>&1; then
    : # 不写，因为 systemd-resolved 已被 mask
  fi
  # 注：/etc/resolv.conf 已由步骤 1 写死为 8.8.8.8，无需 WG DNS 钩子
  ...
  # ── 生成 wg0.conf ────────────────────────────────────
  {
    ...
    [[ -n "${WG_DNS_LINE}" ]] && echo "${WG_DNS_LINE}"
    echo "Table = off"
    echo ""
  ...
  if [[ -n "${PANEL_IP}" ]]; then
    ...
    # 写 hosts（先删旧条目再写新条目）
    grep -v "${panel_host}" /etc/hosts > /tmp/_hk_hosts.tmp && mv /tmp/_hk_hosts.tmp /etc/hosts || true
  ...
  if ! systemctl is-active --quiet wg-quick@wg0 2>/dev/null; then
    ...
    systemctl status wg-quick@wg0 --no-pager -l 2>/dev/null | head -40 || true
    exit 1
  fi

  # ── 三项验证 ─────────────────────────────────────────
  ui_step "步骤 4b：WireGuard 三项验证"
  local ok=1
  ...
  local default_dev
  default_dev="$(ip -4 route show default 2>/dev/null | awk 'NR==1{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}' || true)"
  echo "  出口 IP: ${exit_ip:-?}  地区: ${exit_country:-?}  默认路由dev: ${default_dev:-?}"
  if [[ "${exit_country}" == "US" ]]; then
    ui_ok "出口为美国"
  else
    ui_error "出口非美国（${exit_country:-未知}）"; ok=0
    [[ "${exit_ip}" == "${HK_PUB_IP}" ]] && \
      ui_warn "出口与香港 IP 相同，wg0 未生效（仍本地直出）"
  fi
  ...
  if [[ "${ok}" -eq 0 ]]; then
    ui_error "WireGuard 验证未通过，请检查后重试"
    echo
    echo "  排查命令："
    echo "    journalctl -u wg-quick@wg0 -n 30"
    echo "    wg show"
    echo "    ip route show"
    echo "    ip rule list"
    echo "    ip route get ${US_PUB_IP}"
    exit 1
  fi
...
EOF
  ...
  if /usr/local/bin/update-ai-warp-route.sh >/dev/null 2>&1; then
    ui_ok "sing-box 路由已生成（OpenAI / Claude -> WARP）"
  else
    ui_warn "AI 路由生成失败，回退为基础 direct 配置"
...
_step_panel_ip_monitor() {
  ...
  progress_step 6 "${_STEP_NAME}"

  local panel_host="${PANEL_API_HOST#https://}"
  panel_host="${panel_host#http://}"
  panel_host="${panel_host%%/*}"

  # 修复原 bug：GW 从 state 文件读取，有兜底逻辑
  cat > /usr/local/bin/update-panel-route.sh <<MONITOR
#!/usr/bin/env bash
# update-panel-route.sh — FLYTOex Network
set -euo pipefail
PANEL_HOST="${panel_host}"
...
# 修复：GW 从状态文件读取，不再依赖 eth0rt 路由表（可能为空）
...
if [[ -z "\${WAN_IF}" || -z "\${GW}" ]]; then
  echo "\$(date '+%Y-%m-%d %H:%M:%S') WAN_IF 或 GW 为空，跳过" >> "\${LOG_FILE}"
  exit 0
fi
...
_step_optional_warp() {
  echo
  if ui_confirm "是否现在安装 WARP（Google/Gemini/AI 流量分流）？" "N"; then
    if ! command -v warp_do_install >/dev/null 2>&1; then
      ...
      if [[ -f "${warp_mod}" ]]; then
        ...
        ui_warn "未找到 warp.sh 模块，跳过 WARP 安装"
        ui_warn "事后可运行：flyto.sh warp install"
        return
      fi
    fi
    warp_do_install
  else
    ui_info "已跳过，事后可通过 flyto.sh → WARP 管理 安装"
  fi
}
...
# ============================================================
hk_run_backup() {
  _STEP_NAME="备份 WireGuard 配置"
  ui_step "备份模式"

  # 若 wg0 运行中，暂停以读取本机真实 IP
  local wg_was_up=0
  if systemctl is-active --quiet wg-quick@wg0 2>/dev/null; then
  ...
  local wg_conf="" wg_tmp="" wg_source="" wg_ns="" wg_iface="wg0"

  # 探测 netns（V2bX/sing-box 场景）
  if command -v ip >/dev/null 2>&1 && command -v wg >/dev/null 2>&1; then
    while IFS= read -r ns; do
      ...
      if [[ -n "${ns_ifaces//[[:space:]]/}" ]]; then
        wg_ns="${ns}"; wg_iface="$(awk '{print $1}' <<< "${ns_ifaces}")"; break
      fi
  ...
  if [[ -n "${wg_ns}" ]]; then
    ...
    if ip netns exec "${wg_ns}" wg showconf "${wg_iface}" > "${wg_tmp}" 2>/dev/null; then
      wg_conf="${wg_tmp}"; wg_source="netns ${wg_ns}/${wg_iface}"
      ui_info "WireGuard 运行于 ${wg_source}"
    else
      rm -f "${wg_tmp}"; wg_tmp=""
    fi
  fi
  ...
  if [[ -z "${wg_conf}" ]]; then
    for p in /etc/wireguard/wg0.conf /usr/local/etc/wireguard/wg0.conf; do
      [[ -f "${p}" ]] && { wg_conf="${p}"; wg_source="${p}"; break; }
    done
  fi
  ...
  if [[ -z "${wg_conf}" ]] && command -v wg >/dev/null 2>&1; then
    ...
    if wg showconf wg0 > "${wg_tmp}" 2>/dev/null; then
      wg_conf="${wg_tmp}"; wg_source="wg0 runtime"
    else
      rm -f "${wg_tmp}"; wg_tmp=""
    fi
  fi
  ...
  [[ -z "${addr}" ]] && addr="$(ip -o -4 addr show dev wg0 scope global 2>/dev/null | awk 'NR==1{print $4}' || true)"
  ...
  if [[ -z "${us_tun_ip}" ]] || ! validate_ipv4_cidr "${us_tun_ip}" 2>/dev/null; then
    ...
    validate_ipv4_cidr "${saved_tun}" 2>/dev/null && us_tun_ip="${saved_tun}" && \
      ui_info "US_WG_TUN_IP 来自历史状态：${us_tun_ip}"
  fi
  if [[ -z "${us_tun_ip}" ]] || ! validate_ipv4_cidr "${us_tun_ip}" 2>/dev/null; then
    # 2. 从本地地址推断
    if [[ -n "${addr}" ]]; then
      ...
      if validate_ipv4_cidr "${guessed}" 2>/dev/null; then
        us_tun_ip="${guessed}"
  ...
  if [[ -z "${node_id}" ]]; then
    for cfg in /etc/V2bX/config.yml /usr/local/etc/V2bX/config.yml; do
      [[ -f "${cfg}" ]] && node_id="$(sed -nE 's/^[[:space:]]*NodeID:[[:space:]]*([0-9]+).*$/\1/p' "${cfg}" 2>/dev/null | head -1)" && [[ -n "${node_id}" ]] && break
    done
  fi
  ...
  # 占位值处理
  _hk_is_placeholder "${priv}"      && { ui_warn "HK_PRIV_KEY 未读取到";      priv="REPLACE_WITH_HK_PRIV_KEY"; }
  _hk_is_placeholder "${addr}"      && { ui_warn "HK_WG_ADDR 未读取到";       addr="REPLACE_WITH_HK_WG_ADDR"; }
  _hk_is_placeholder "${peer_pub}"  && { ui_warn "HK_WG_PEER_PUBKEY 未读取到"; peer_pub="REPLACE_WITH_HK_WG_PEER_PUBKEY"; }
  _hk_is_placeholder "${endpoint}"  && { ui_warn "HK_WG_ENDPOINT 未读取到";   endpoint="REPLACE_WITH_HK_WG_ENDPOINT"; }
  _hk_is_placeholder "${us_tun_ip}" || [[ "${us_tun_ip}" == "/32" || -z "${us_tun_ip}" ]] && {
    ui_warn "US_WG_TUN_IP 未读取到，请手动补全"
    us_tun_ip="REPLACE_WITH_US_WG_TUN_IP"
  }
  [[ -z "${node_id}" ]] && { ui_warn "V2bX Node ID 未读取到"; node_id="REPLACE_WITH_NODE_ID"; }

  # 输出备份块
  echo
  echo -e "  ${Y}╔═══════════════════════════════════════════════════════════╗${N}"
  echo -e "  ${Y}║  ⚠  请完整复制 BEGIN 到 END 之间的内容并妥善保存         ║${N}"
  echo -e "  ${Y}╚═══════════════════════════════════════════════════════════╝${N}"
  echo
  cat <<BACKUP
########## BEGIN FLYTO BACKUP ##########
...
hk_run_fresh() {
  ...
  _step_collect_network
  progress_gate "步骤 2 完成，继续步骤 3？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _input_wg_fresh
  progress_gate "步骤 3 完成，继续步骤 4？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _step_setup_wireguard
  progress_gate "步骤 4 完成，继续步骤 5？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _step_install_v2bx
  progress_gate "步骤 5 完成，继续步骤 6？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _step_panel_ip_monitor
  progress_gate "步骤 6 完成，继续可选 WARP 安装？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0
...
hk_run_restore() {
  ...
  _input_wg_restore
  progress_gate "恢复块解析完成，继续网络信息采集？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  # 若备份中有网络信息且与当前机器 IP 一致则直接用，否则重新采集
  local current_pub
  ...
  if [[ -n "${HK_WAN_IF}" && -n "${HK_GW}" && -n "${HK_PUB_IP}" \
    ...
    printf '%s\n' "${HK_GW}"     > "${HK_STATE_DIR}/gateway"
    printf '%s\n' "${HK_PUB_IP}" > "${HK_STATE_DIR}/pub_ip"
    ui_ok "使用备份中的网络信息（当前 IP 一致）：${HK_WAN_IF} / ${HK_GW} / ${HK_PUB_IP}"
  else
    [[ -n "${current_pub}" && "${current_pub}" != "${HK_PUB_IP:-}" ]] && \
      ui_warn "当前机器 IP（${current_pub}）与备份 IP（${HK_PUB_IP:-?}）不一致，将重新采集网络信息"
    _step_collect_network
  fi

  progress_gate "网络信息就绪，继续配置 WireGuard？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _step_setup_wireguard
  progress_gate "WireGuard 完成，继续安装 V2bX？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _step_install_v2bx
  progress_gate "V2bX 完成，继续部署面板 IP 监控？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0

  _step_panel_ip_monitor
  progress_gate "面板监控完成，继续可选 WARP 安装？" || gate_rc=$?
  [[ "${gate_rc}" -eq 2 ]] && return 0
...
hk_run_install() {
  ...
  local mode=""
  ui_menu mode "安装模式选择" "请选择部署方式" \
    "1" "全新安装（逐字段输入 WireGuard 配置）" \
    "2" "恢复模式（粘贴备份块一键恢复）" \
    "0" "返回" || return 0
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  if [[ -z "${PANEL_API_HOST:-}" || -z "${PANEL_API_KEY:-}" ]]; then
    echo "[ERR] 独立运行需设置环境变量："
    echo "  export PANEL_API_HOST=https://your-panel.example.com"
    echo "  export PANEL_API_KEY=your-api-key"
    exit 1
  fi
  case "${1:-menu}" in
