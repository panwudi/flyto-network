...
香港中转节点部署工具，集成 WireGuard 中转配置、V2bX 代理节点管理、Cloudflare WARP 透明代理（可选），支持配置加密保存与一键恢复。
...
在香港服务器上安装 Cloudflare WARP，通过 iptables + ipset 将所有发往 Google IP 段的 TCP 流量自动透明转发，服务器上任何程序访问 Google/Gemini **无需修改代码、无需配置环境变量**。
